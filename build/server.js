...
function validateHex(hexStr, length) {
    if (typeof hexStr !== 'string')
        throw new Error('Hex must be string');
    if (hexStr.length % 2)
        throw new Error('Hex must have even length');
    if (length && hexStr.length !== length * 2)
        throw new Error(`Hex must be ${length * 2} chars long`);
    if (!/^[0-9a-f]*$/i.test(hexStr))
        throw new Error('Hex must contain only 0-9 and a-f');
}
// --- Tool Definitions ---
server.tool("generate_address", "Generates a P2WPKH address from a private key (WIF format).", {
...
server.tool("sign_transaction", "Signs a partially signed Bitcoin transaction (PSBT).", {
